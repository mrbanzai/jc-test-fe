...
class JobsController extends Skookum_Controller_Action
{
    ...
    // check for matching jobs in a given category or location
		...
		if (!empty($job['category'])) {
		    $jobsInCategory = $this->Api->get('/jobs/search/',array('category'=>$job['category'],'perPage'=>'10'));
		    $this->view->jobsInCategory = $jobsInCategory['data']['results'];
		}

		// make jobs in location available
		if (!empty($job['location'])) {
			// The API foolishly requires the perPage value to be "standard", so we'll have to make it
			// 10 instead of 11
			$jobsInLocation = $this->Api->get('/jobs/search/',array('location'=>$job['location'],'perPage'=>'10'));
		...
		if (!empty($job['state'])) {
			// The API foolishly requires the perPage value to be "standard", so we'll have to make it
			// 10 instead of 11
			$jobsInState = $this->Api->get('/jobs/search/',array('state'=>$job['state'],'perPage'=>'10'));
			$this->view->jobsInState = $jobsInState['data']['results'];
		}
	...
	protected function _handleEmail($toMyself = true, $job, $post)
	{
		...
		$mail->setFrom($config['replyToEmail'], $config['replyToName'])
			 ->addTo($post['email'])
			 ->setSubject('Apollo Group Job Search');
	...
	Your friend recently searched the Apollo Group website and thought that you may be interested in the job below.
...
MAILHTML
	...
	Thank you for your interest in a career with Apollo Group. Below you will find a link to the job that you found on the mobile website.
